...
def _save_comparison_plot(
    ...
    """Save a bar chart comparing ideal, noisy, and CDR-mitigated <ZZ> values."""
    ...
    ax.set_title("Clifford Data Regression (CDR) — Bell State ⟨ZZ⟩",
    ...
    fig.tight_layout()
    fig.savefig(output_dir / f"{filename}_cdr_comparison.png", dpi=150)


def main():
    circuit = create_bell_state()
    save_circuit(circuit, "bell_state_cdr", category="ideal")

    ideal_counts = run_circuit(circuit)
    save_histogram(ideal_counts, "bell_state_cdr", category="ideal")
    ...
    save_histogram(noisy_counts, "bell_state_cdr", category="noisy")
    noisy_ev = zz_expectation(noisy_counts)

    mitigated_ev = run_cdr(
        circuit,
    ...
    print("=" * 55)
    print("Clifford Data Regression (CDR) — Bell State")
    print("=" * 55)
    print(f"\nNoise model         : Depolarizing, p = {error_probability:.1%}")
    print(f"Training circuits   : 10  (fraction non-Clifford: 10%)")
    print()
    print(f"{'Metric':<30} {'Value':>10}")
    print("-" * 42)
    print(f"{'Ideal <ZZ>':<30} {ideal_ev:>10.4f}")
    print(f"{'Noisy <ZZ>':<30} {noisy_ev:>10.4f}")
    print(f"{'CDR Mitigated <ZZ>':<30} {mitigated_ev:>10.4f}")
